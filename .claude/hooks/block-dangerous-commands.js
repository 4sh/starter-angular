...
const PATTERNS = [
  ...
  { level: 'high', id: 'git-add-all',    regex: /\bgit\s+add\s+(-\w*A\w*\b|--all\b|\.\/?(\s|$|[;&|]))/,                   reason: 'git add -A / git add . forbidden — stage files by name' },
  { level: 'high', id: 'git-no-verify',  regex: /\bgit\s+(commit|push)\b.*--no-(verify|gpg-sign)\b/,                      reason: 'skipping git hooks is forbidden' },
  { level: 'high', id: 'git-discard-all', regex: /\bgit\s+(checkout|restore)\s+(--\s+)?\.\/?(\s|$|[;&|])/,                reason: 'discards all uncommitted changes' },
  { level: 'high', id: 'chmod-777',      regex: /\bchmod\b.+\b777\b/,                                                     reason: 'chmod 777 is a security risk' },
  { level: 'high', id: 'cat-env',        regex: /\b(cat|less|head|tail|more|bat)\s+\S*\.env(?!\.(example|sample|template))(\.\w+)?\b/, reason: 'reading .env file exposes secrets' },
