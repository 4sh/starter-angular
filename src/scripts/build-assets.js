...
function registerImage(filename, theme, type, mode) {
  ...
  if (map[filename][theme][mode]) {
    console.warn(
      `⚠️  WARNING: Duplicate detected for ${filename} (Theme: ${theme}, Mode: ${mode})`,
    );
  }
  ...
  Object.keys(map).forEach((filename) => {
    ...
    Object.keys(imageEntry).forEach((theme) => {
      ...
      if (hasBase && (hasLight || hasDark)) {
        errorList.push(
          `[${theme}/${filename}] Ambiguous: image present at root (base) AND in a mode folder.`,
        );
      }
      ...
      if (hasLight && !hasDark) {
        errorList.push(
          `[${theme}/${filename}] Incomplete: 'light' version found without a 'dark' version.`,
        );
      }
      if (!hasLight && hasDark) {
        errorList.push(
          `[${theme}/${filename}] Incomplete: 'dark' version found without a 'light' version.`,
        );
      }
    });
    ...
    if (!imageEntry['common']) {
      ...
      if (missingInThemes.length > 0) {
        ...
        errorList.push(
          `[${filename}] Coverage gap: image absent from 'common' and missing in: ${missingInThemes.join(', ')}.`,
        );
      }
    }
  });
