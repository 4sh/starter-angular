...
const meta: Meta<UiInputMask> = {
  ...
  argTypes: {
    ...
    mask: { control: 'text', description: '9 digit · a letter · * alphanumeric · other = literal.', table: { type: { summary: 'string' } } },
    ranges: { control: 'text', description: 'Bounds per numeric segment, in mask order: "0-23 0-59". `*` = unbounded.', table: { type: { summary: 'string' } } },
    slotChar: { control: 'text', table: { type: { summary: 'string' }, defaultValue: { summary: '"_"' } } },
    unmask: { control: 'boolean', description: 'Emits the raw value (without literals).', table: { defaultValue: { summary: 'false' } } },
