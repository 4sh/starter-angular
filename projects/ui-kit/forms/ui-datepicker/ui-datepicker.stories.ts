...
const meta: Meta<UiDatepicker> = {
  ...
  argTypes: {
    ...
    valueType: {
      ...
      options: ['date', 'iso'],
      description: '**Required, no default** — forces an explicit choice. Type of the value **emitted** by `valueChange`/`dateSelect`: `\'date\'` — a `Date`; `\'iso\'` — a `"yyyy-MM-dd"` string. `writeValue` always accepts either.',
    ...
    view: { control: 'inline-radio', options: ['date', 'month', 'year'], description: 'Base granularity (also MonthPicker/YearPicker).', table: { type: { summary: 'DatepickerView' }, defaultValue: { summary: '"date"' } } },
    ...
    showIcon: { control: 'boolean', description: "Shows the trigger's toggle button (calendar/clock).", table: { type: { summary: 'boolean' }, defaultValue: { summary: 'true' } } },
    ...
    minDate: { control: 'text', description: 'Minimum selectable date. `Date` or ISO `"yyyy-MM-dd"`.', table: { type: { summary: 'Date | string | null' } } },
    maxDate: { control: 'text', description: 'Maximum selectable date. `Date` or ISO `"yyyy-MM-dd"`.', table: { type: { summary: 'Date | string | null' } } },
    disabledDates: { control: false, description: 'Specific disabled dates. `Date` or ISO `"yyyy-MM-dd"`, mixable.', table: { type: { summary: '(Date | string)[]' } } },
    disabledDays: { control: false, description: 'Disabled days of the week (0 = Sunday … 6 = Saturday).', table: { type: { summary: 'number[]' } } },
    dateFormat: { control: false, description: "Custom display formatter (symmetric to `parseDate`).", table: { type: { summary: '(date: Date) => string' } } },
    parseDate: { control: false, description: 'Custom parser for keyboard input (symmetric to `dateFormat`).', table: { type: { summary: '(value: string) => Date | null' } } },
    ...
    stepMinute: { control: { type: 'number', min: 1, max: 30 }, description: "Minute increment on the chevrons / arrow keys (typing stays exact).", table: { type: { summary: 'number' }, defaultValue: { summary: '1' } } },
    editableTime: { control: 'boolean', description: 'Allows typing hours / minutes (AM/PM stays a toggle).', table: { type: { summary: 'boolean' }, defaultValue: { summary: 'true' } } },
    showButtonBar: { control: 'boolean', table: { type: { summary: 'boolean' }, defaultValue: { summary: 'false' } } },
    todayLabel: { control: 'text', description: 'Label of the today button.', table: { type: { summary: 'string' }, defaultValue: { summary: '"Today"' } } },
    clearLabel: { control: 'text', description: 'Label of the clear button (and the `showClear` cross).', table: { type: { summary: 'string' }, defaultValue: { summary: '"Clear"' } } },
    inline: { control: 'boolean', table: { type: { summary: 'boolean' }, defaultValue: { summary: 'false' } } },
    showClear: { control: 'boolean', description: 'Shows a cross to clear the value when it is set.', table: { type: { summary: 'boolean' }, defaultValue: { summary: 'false' } } },
    autoFlip: { control: 'boolean', description: "Flips the panel upward if there's no room below.", table: { type: { summary: 'boolean' }, defaultValue: { summary: 'true' } } },
    closeOnSelect: { control: 'boolean', table: { type: { summary: 'boolean' }, defaultValue: { summary: 'true' } } },
    allowInput: { control: 'boolean', description: 'Allows typing the date in the field (single mode, outside timeOnly). Parsed on blur / Enter.', table: { type: { summary: 'boolean' }, defaultValue: { summary: 'false' } } },
    panelStyleClass: { control: 'text', description: 'Additional class(es) applied to the panel (scoped customization).', table: { type: { summary: 'string' } } },
