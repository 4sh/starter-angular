import type { Meta, StoryObj } from '@storybook/angular';
import { moduleMetadata } from '@storybook/angular';
import { UiReadOnly } from '@4sh/ui-kit/informative/ui-read-only';
import { UiBadge } from '@4sh/ui-kit/informative/ui-badge';
import { UiInput } from '@4sh/ui-kit/forms/ui-input';

const meta: Meta<UiReadOnly> = {
  title: 'Components/ui/informative/ui-read-only',
  component: UiReadOnly,
  decorators: [moduleMetadata({ imports: [UiReadOnly, UiBadge, UiInput] })],
  parameters: {
    layout: 'centered',
    design: {
      type: 'figma',
      url: 'https://www.figma.com/design/GZww5hdUA49LB8XWeWP6tl/-Projet----UI-Kit?node-id=3205-2882&t=QwmSe7YP798RcocA-1',
    },
  },
  argTypes: {
    label: {
      control: { type: 'text' },
      description: 'Field label (raw string — i18n is the caller\'s responsibility).',
      table: { type: { summary: 'string' }, defaultValue: { summary: 'undefined' } },
    },
    value: {
      control: { type: 'text' },
      description: 'Displayed value. Ignored if content is projected via `<ng-content>`.',
      table: { type: { summary: 'string | number | null | undefined' }, defaultValue: { summary: 'undefined' } },
    },
    required: {
      control: { type: 'boolean' },
      description: 'Shows the required marker (*) on the label (visual parity with a required field).',
      table: { type: { summary: 'boolean' }, defaultValue: { summary: 'false' } },
    },
    labelTemplate: {
      control: false,
      description: 'Fully custom label markup (`TemplateRef`), replaces the default `ui-label`.',
      table: { type: { summary: 'TemplateRef' }, defaultValue: { summary: 'undefined' } },
    },
    fallback: {
      control: { type: 'text' },
      description: 'Muted text shown when the value is empty/null.',
      table: { type: { summary: 'string' }, defaultValue: { summary: '"-"' } },
    },
    emptyLabel: {
      control: { type: 'text' },
      description: 'Accessible text announced instead of the `fallback` symbol when the value is empty (e.g. « Not provided »). `fallback` stays decorative.',
      table: { type: { summary: 'string' }, defaultValue: { summary: 'undefined' } },
    },
    layout: {
      control: { type: 'inline-radio' },
      options: ['vertical', 'horizontal', 'grid'],
      description: 'Layout: label above (`vertical`), beside (`horizontal`), or driven by a grid system (`grid` + `rowClass`/`labelClass`/`valueClass`).',
      table: { type: { summary: 'ReadOnlyLayout' }, defaultValue: { summary: '"vertical"' } },
    },
    matchField: {
      control: { type: 'boolean' },
      description: 'Adapts the layout to sit alongside fields: matches a `ui-field`\'s label→value gap AND height (value centered, no horizontal padding). Disabled by default — a standalone read-only stays compact.',
      table: { type: { summary: 'boolean' }, defaultValue: { summary: 'false' } },
    },
    rowClass: {
      control: { type: 'text' },
      description: 'Classes added to the row (`<dl>`), e.g. a grid row class like `flex-x`.',
      table: { type: { summary: 'string' }, defaultValue: { summary: 'undefined' } },
    },
    labelClass: {
      control: { type: 'text' },
      description: 'Classes added to the label cell (`<dt>`), e.g. `cell phone-24 desktop-auto`.',
      table: { type: { summary: 'string' }, defaultValue: { summary: 'undefined' } },
    },
    valueClass: {
      control: { type: 'text' },
      description: 'Classes added to the value cell (`<dd>`), e.g. `cell phone-24 desktop-auto`.',
      table: { type: { summary: 'string' }, defaultValue: { summary: 'undefined' } },
    },
    size: {
      control: { type: 'inline-radio' },
      options: ['default', 'small'],
      description: 'Text size.',
      table: { type: { summary: 'ReadOnlySize' }, defaultValue: { summary: '"default"' } },
    },
    labelAlign: {
      control: { type: 'inline-radio' },
      options: ['left', 'right'],
      description: 'Label alignment (horizontal layout only).',
      table: { type: { summary: 'ReadOnlyAlign' }, defaultValue: { summary: '"left"' } },
    },
    labelWidth: {
      control: { type: 'text' },
      description: 'Width of the label column in horizontal layout (CSS length, e.g. `160px`). Empty = fits content.',
      table: { type: { summary: 'string' }, defaultValue: { summary: 'undefined' } },
    },
    multiline: {
      control: { type: 'boolean' },
      description: 'Preserves the value\'s line breaks (`white-space: pre-line`).',
      table: { type: { summary: 'boolean' }, defaultValue: { summary: 'false' } },
    },
    inline: {
      control: { type: 'boolean' },
      description: 'Fits its content and flows inline instead of filling the container.',
      table: { type: { summary: 'boolean' }, defaultValue: { summary: 'false' } },
    },
  },
  args: {
    label: 'Email address',
    value: 'jean.dupont@4sh.fr',
    fallback: '-',
    layout: 'vertical',
    size: 'default',
    labelAlign: 'left',
    multiline: false,
    inline: false,
  },
};

export default meta;
type Story = StoryObj<UiReadOnly>;

// Disposition verticale (label au-dessus de la valeur)
export const Vertical: Story = {};

// Disposition horizontale (label à côté de la valeur)
export const Horizontal: Story = {
  args: { layout: 'horizontal' },
};

// Colonne de label fixe (alignement de plusieurs lignes)
export const HorizontalFixedLabel: Story = {
  name: 'Horizontal - fixed column',
  render: (args) => ({
    props: args,
    template: `
      <div style="display:flex; flex-direction:column; gap:12px; min-width:320px;">
        <ui-read-only layout="horizontal" labelWidth="140px" labelAlign="right" label="Name" value="John Doe" />
        <ui-read-only layout="horizontal" labelWidth="140px" labelAlign="right" label="E-mail" value="jean.dupont@4sh.fr" />
        <ui-read-only layout="horizontal" labelWidth="140px" labelAlign="right" label="Role" value="Design System Engineer" />
      </div>
    `,
  }),
};

// Valeur vide → repli atténué
export const Fallback: Story = {
  args: { label: 'Phone', value: null },
};

// Valeur vide annoncée aux lecteurs d'écran (« - » reste décoratif)
export const EmptyAccessible: Story = {
  name: 'Accessible collapse',
  args: { label: 'Phone', value: null, emptyLabel: 'Not provided' },
};

// Contrôle total du layout via un système de grille (Gridaflex)
export const GridLayout: Story = {
  name: 'Grid (full control)',
  render: (args) => ({
    props: args,
    template: `
      <div style="min-width:520px;">
        <ui-read-only
          layout="grid"
          rowClass="flex-x flex-gap-x"
          labelClass="cell phone-24 desktop-6"
          valueClass="cell phone-24 desktop-18"
          label="Label"
          value="Label column 6/24, value 18/24 on desktop - full width on mobile" />
      </div>
    `,
  }),
};

// Alignement avec un champ de formulaire voisin (matchField, opt-in)
export const NextToField: Story = {
  name: 'Next to a field',
  render: (args) => ({
    props: args,
    template: `
      <div class="flex-x flex-gap-x" style="min-width:560px;">
        <div class="cell phone-24 desktop-12">
          <ui-read-only matchField label="Reference" value="REF-2026-0042" />
        </div>
        <div class="cell phone-24 desktop-12">
          <ui-input label="Comment" placeholder="Type…" />
        </div>
      </div>
    `,
  }),
};

// Taille réduite
export const Small: Story = {
  args: { size: 'small' },
};

// Valeur multiligne
export const Multiline: Story = {
  args: {
    label: 'Address',
    value: '12 Market Street\n75002 Paris\nFrance',
    multiline: true,
  },
};

// Contenu projeté (valeur riche : badge, lien…)
export const ProjectedContent: Story = {
  name: 'Projected content',
  render: (args) => ({
    props: args,
    template: `
      <ui-read-only label="Status">
        <ui-badge level="success" value="Active" icon="check" />
      </ui-read-only>
    `,
  }),
};

// Inline mode (fits into the text flow)
export const Inline: Story = {
  render: (args) => ({
    props: args,
    template: `
      <p style="max-width:420px; line-height:1.6;">
        Last connection&nbsp;:
        <ui-read-only inline layout="horizontal" label="on" value="24 July 2026" />
        from Paris.
      </p>
    `,
  }),
};

// Exemple composé : fiche récapitulative
export const Summary: Story = {
  name: 'Example - summary card',
  render: (args) => ({
    props: args,
    template: `
      <div style="display:flex; flex-direction:column; gap:16px; min-width:360px;">
        <ui-read-only label="Full name" value="John Doe" />
        <ui-read-only label="E-mail" value="jean.dupont@4sh.fr" />
        <ui-read-only label="Phone" value="" />
        <ui-read-only label="Status">
          <ui-badge level="success" value="Active" icon="check" />
        </ui-read-only>
      </div>
    `,
  }),
};
