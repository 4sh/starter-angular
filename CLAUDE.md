...
by `pnpm docs:config` (`scripts/docs.config.mjs` → `storybook/generated/ui-config.json`) and
...
- `pnpm docs:config --check` (or `pnpm docs:config:check`) fails if the manifest is stale;
...
Build: `pnpm tokens:build` (`scripts/tokens.build.mjs`, **Style Dictionary v5** engine) →
