...
**The `.scss` comments are what write the doc**: `///` = documented public contract,
`//` = internal note (invisible in the doc). Both in **English** — see the documentation
language rule below. The `///` goes **at the end of the declaration**, vertically aligned
with its visual group:
...
### Documentation language: English is the source (FSHSP-87)
